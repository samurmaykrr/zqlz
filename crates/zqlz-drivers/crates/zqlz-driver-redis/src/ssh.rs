//! Redis SSH Tunnel Support
//!
//! Provides SSH tunnel functionality for secure Redis connections
...
//! module but is tailored for Redis's default port and connection patterns.
...
/// Error types for Redis SSH tunnel operations
#[derive(Debug, thiserror::Error)]
pub enum RedisSshTunnelError {
...
/// An SSH tunnel for Redis connections
///
/// This struct manages an SSH session and provides local port forwarding
/// to a remote Redis server. When dropped, all resources are cleaned up.
///
/// # Example
///
/// ```ignore
/// use zqlz_drivers::redis::RedisSshTunnel;
/// use zqlz_core::security::SshTunnelConfig;
///
/// let config = SshTunnelConfig::with_password("bastion.example.com", "user", "password");
/// let tunnel = RedisSshTunnel::new(&config, "db.internal", 6379)?;
///
/// // Connect to the tunnel's local port
/// let url = format!("redis://user:pass@127.0.0.1:{}/mydb", tunnel.local_port());
/// ```
pub struct RedisSshTunnel {
    session: Session,
    local_port: u16,
    remote_host: String,
    remote_port: u16,
...
impl std::fmt::Debug for RedisSshTunnel {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RedisSshTunnel")
...
impl RedisSshTunnel {
    ...
    /// Create a new SSH tunnel for Redis
    ///
    /// Establishes an SSH connection to the configured host and sets up
    /// local port forwarding to the remote Redis server.
    ///
    /// # Arguments
    ///
    /// * `config` - SSH tunnel configuration
    /// * `remote_host` - The Redis server hostname (as seen from the SSH server)
    /// * `remote_port` - The Redis server port (typically 6379)
    ///
    /// # Returns
    ///
    /// A new `RedisSshTunnel` with a local port ready for connections.
    ///
    /// # Example
    ///
    /// ```ignore
    /// let config = SshTunnelConfig::with_password("bastion.example.com", "user", "password");
    /// let tunnel = RedisSshTunnel::new(&config, "redis.internal", 6379)?;
    ...
    pub fn new(config: &SshTunnelConfig, remote_host: &str, remote_port: u16) -> Result<Self> {
        ...
        .map_err(|e| RedisSshTunnelError::ConnectionFailed {
        ...
        session.handshake().map_err(|e| {
            RedisSshTunnelError::HandshakeFailed(format!("SSH handshake failed: {}", e))
        })?;
    ...
    /// Create a new SSH tunnel using the default Redis port (6379)
    ///
    /// This is a convenience method that calls `new()` with port 6379.
    pub fn with_default_port(config: &SshTunnelConfig, remote_host: &str) -> Result<Self> {
        Self::new(config, remote_host, Self::DEFAULT_REDIS_PORT)
    }

    /// Get the local port number
    ///
    /// Use this port to connect to the tunneled Redis server.
    ...
    /// Get a Redis connection URL for this tunnel
    ///
    /// Returns a connection URL suitable for redis_async.
    ///
    /// # Arguments
    ///
    /// * `user` - Redis username
    /// * `password` - Redis password (will be URL-encoded if needed)
    /// * `database` - Database name
    ///
    /// # Example
    ///
    /// ```ignore
    /// let url = tunnel.redis_url("root", "secret", "myapp");
    /// // Returns: redis://root:secret@127.0.0.1:12345/myapp
    /// ```
    pub fn redis_url(&self, user: &str, password: &str, database: &str) -> String {
        format!(
            "redis://{}:{}@127.0.0.1:{}/{}",
    ...
    /// Get a Redis connection URL without credentials
    ///
    /// Returns a connection URL template without embedded credentials.
    /// Useful when credentials are provided separately.
    pub fn redis_url_template(&self, database: &str) -> String {
        format!("redis://127.0.0.1:{}/{}", self.local_port, database)
    }
}

impl Drop for RedisSshTunnel {
    fn drop(&mut self) {
        info!(local_port = self.local_port, "Closing Redis SSH tunnel");
...
fn authenticate(session: &mut Session, username: &str, auth: &SshAuthMethod) -> Result<()> {
    match auth {
        SshAuthMethod::Password { password } => {
            ...
            session
                ...
                .map_err(|e| RedisSshTunnelError::AuthenticationFailed(e.to_string()))?;
        }
        SshAuthMethod::PrivateKey { path, passphrase } => {
            debug!(path = %path.display(), "Authenticating with private key");
            if !path.exists() {
                return Err(RedisSshTunnelError::PrivateKeyNotFound {
            ...
            session
                ...
                .map_err(|e| RedisSshTunnelError::InvalidPrivateKey(e.to_string()))?;
        }
        SshAuthMethod::Agent => {
    ...
    if !session.authenticated() {
        return Err(RedisSshTunnelError::AuthenticationFailed(
...
fn authenticate_with_agent(session: &mut Session, username: &str) -> Result<()> {
    let mut agent = session
        .agent()
        .map_err(|e| RedisSshTunnelError::AgentNotAvailable(e.to_string()))?;
    ...
    agent.list_identities().map_err(|e| {
        RedisSshTunnelError::AgentNotAvailable(format!("Failed to list identities: {}", e))
    })?;
    ...
    if identities.is_empty() {
        return Err(
            RedisSshTunnelError::AgentNotAvailable("No identities in agent".to_string()).into(),
        );
    }
    ...
    Err(RedisSshTunnelError::AuthenticationFailed("No agent identity worked".to_string()).into())
}
...
fn handle_connection(
    ...
    let mut channel = session
        ...
        .map_err(|e| RedisSshTunnelError::PortForwardingFailed(e.to_string()))?;
