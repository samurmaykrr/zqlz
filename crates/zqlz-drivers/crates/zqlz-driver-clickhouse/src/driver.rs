...
use zqlz_core::{
    ...
    FunctionCategory, KeywordCategory, KeywordInfo, QueryResult, Result, Row, SqlFunctionInfo,
    SqlObjectName, StatementResult, TableOptionDef, TableOptionType, Transaction, Value, ZqlzError,
};
...
impl Connection for ClickHouseConnection {
    ...
    fn drop_table_sql(
        ...
        if options.cascade {
            return Err(ZqlzError::NotSupported(
                "ClickHouse does not support DROP TABLE ... CASCADE".to_string(),
            ));
        }
        Ok(sql)
    }
    ...
    fn drop_view_sql(&self, view_name: &SqlObjectName, options: DropViewOptions) -> Result<String> {
        ...
        if options.cascade {
            return Err(ZqlzError::NotSupported(
                "ClickHouse does not support DROP VIEW ... CASCADE".to_string(),
            ));
        }
        Ok(sql)
    }
    ...
    fn drop_trigger_sql(
        ...
        Err(ZqlzError::NotSupported(
            "ClickHouse does not support triggers".to_string(),
        ))
    }
