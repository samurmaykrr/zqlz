...
struct SelectOptions {
    ...
    menu_width: Length,
    /// Minimum width for the dropdown menu (default: None, uses parent width)
    menu_min_width: Option<Pixels>,
    disabled: bool,
    appearance: bool,
    /// Whether to show focus border when focused (default: true)
    focus_border: bool,
}
    ...
    D: SelectDelegate + 'static,
{
    ...
    /// When set, the dropdown will expand to fit its content but not be smaller than this.
    /// This overrides the default behavior of using the parent width.
    pub fn menu_min_width(mut self, min_width: Pixels) -> Self {
        self.options.menu_min_width = Some(min_width);
        self
    }
    ...
    /// Set whether to show focus border when focused (default: true).
    ///
    /// When false, no focus border/outline is shown even when the select is focused.
    /// Useful for inline editing scenarios where focus border would look out of place.
    pub fn focus_border(mut self, focus_border: bool) -> Self {
        self.options.focus_border = focus_border;
        self
    }
}
