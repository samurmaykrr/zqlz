...
impl SyntaxHighlighter {
    ...
    pub fn new(lang: &str) -> Self {
        ...
        match Self::build_combined_injections_query(lang) {
            Ok(result) => {
                tracing::debug!(
                    language = lang,
                    "SyntaxHighlighter initialization successful"
                );
                result
            }
            Err(err) => {
                tracing::warn!(language = lang, error = %err, "SyntaxHighlighter init failed, falling back to text mode");
    ...
    fn build_combined_injections_query(lang: &str) -> Result<Self> {
        ...
        tracing::debug!(
            language = lang,
            highlights_len = config.highlights.len(),
            highlights_preview = &config.highlights.chars().take(200).collect::<String>(),
            "Language config found"
        );
        ...
        tracing::trace!("Creating Query object with combined source");
        let query = match Query::new(&config.language, &query_source) {
            ...
            Err(e) => {
                tracing::error!(error = ?e, "Query creation failed");
                return Err(anyhow!("new query failed: {:?}", e));
            }
        };
    ...
    pub fn update(&mut self, edit: Option<InputEdit>, text: &Rope) {
        ...
        // Debug: Print parse tree for SQL
    ...
    ///
    /// ```no_run
    /// use zqlz_ui::widgets::highlighter::{HighlightTheme, SyntaxHighlighter};
    /// use ropey::Rope;
    ///
    /// let code = "fn main() {\n    println!(\"Hello\");\n}";
...
fn print_tree_node(node: &tree_sitter::Node, source: &Rope, indent: usize) {
    let start = node.start_byte();
    let end = node.end_byte();
    let text: String = source.to_string();
    let text_slice = if start < text.len() && end <= text.len() {
        &text[start..end]
    } else {
        ""
    };
    let preview = if text_slice.len() > 40 {
        format!("{}...", &text_slice[..40])
    } else {
    ...
    // Only print first level children to avoid too much output
    if indent < 3 {
        for i in 0..node.child_count() as u32 {
            if let Some(child) = node.child(i) {
                print_tree_node(&child, source, indent + 1);
            }
        }
    }
}
