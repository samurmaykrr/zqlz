...
impl DiagnosticSet {
    ...
    ///
    /// Returns a tuple of (errors, warnings, hints/infos)
    ...
    /// Get set of line numbers (0-indexed) that have diagnostics
    pub fn lines_with_diagnostics(&self) -> std::collections::HashSet<usize> {
        let mut lines = std::collections::HashSet::new();
        for entry in self.diagnostics.iter() {
            // The diagnostic.range contains Position with line info
            lines.insert(entry.diagnostic.range.start.line as usize);
        }
        lines
    }

    /// Get the most severe diagnostic level for a specific line (0-indexed)
    pub fn severity_for_line(&self, line: usize) -> Option<DiagnosticSeverity> {
        let mut max_severity: Option<DiagnosticSeverity> = None;

        for entry in self.diagnostics.iter() {
            if entry.diagnostic.range.start.line as usize == line {
                let severity = entry.diagnostic.severity;
                match (&max_severity, severity) {
                    (None, _) => max_severity = Some(severity),
                    (Some(DiagnosticSeverity::Error), _) => {}
                    (_, DiagnosticSeverity::Error) => max_severity = Some(severity),
                    (Some(DiagnosticSeverity::Warning), _) => {}
                    (_, DiagnosticSeverity::Warning) => max_severity = Some(severity),
                    _ => {}
                }
            }
        }
    ...
    /// Get all diagnostics as a vector for external use (e.g., Problems panel)
...
#[cfg(test)]
mod tests {
    use crate::widgets::input::Position;
