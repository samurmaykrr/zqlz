...
impl LanguageConfig {
    pub fn new(
        ...
        Self {
            ...
            brackets: SharedString::from(brackets.to_string()),
            folds: SharedString::from(folds.to_string()),
        }
    }
}
...
impl SyntaxColors {
    pub fn style(&self, name: &str) -> Option<HighlightStyle> {
        ...
        let style = match name {
            ...
            "constant.builtin" => self.constant, // Fallback to constant
            "constructor" => self.constructor,
            "embedded" => self.embedded,
            "emphasis" => self.emphasis,
            "emphasis.strong" => self.emphasis_strong,
            "enum" => self.enum_,
            "field" => self.property, // Map field to property
            "function" => self.function,
            "function.call" => self.function, // Map function.call to function
            "function.builtin" => self.function, // Map function.builtin to function
            "hint" => self.hint,
            "keyword" => self.keyword,
            "keyword.operator" => self.keyword, // Fallback to keyword
            "label" => self.label,
            ...
            "storageclass" => self.keyword, // Map storageclass to keyword
            "spell" => None,               // Ignore spell checking annotations
            _ => None,
        }
        .map(|s| s.into());
...
impl LanguageRegistry {
    ...
    pub fn singleton() -> &'static LazyLock<LanguageRegistry> {
        static INSTANCE: LazyLock<LanguageRegistry> = LazyLock::new(|| {
            ...
            languages::register_sql_dialects(&registry);
            languages::register_nosql_dialects(&registry);
            registry
        });
        &INSTANCE
    }
...
#[cfg(test)]
mod tests {
    use crate::widgets::highlighter::LanguageConfig;
