...
impl DockArea {
    ...
    /// Get the center TabPanel entity if it exists.
    ///
    /// This is useful for subscribing to events from the center tab panel,
    /// such as TabContextMenuEvent when a user right-clicks on a tab.
    ///
    /// Returns None if the center item is not a Tabs layout.
    pub fn center_tab_panel(&self) -> Option<Entity<TabPanel>> {
        match &self.items {
            DockItem::Tabs { view, .. } => Some(view.clone()),
            _ => None,
        }
    ...
    /// Get the active panel in the center dock area.
    ///
    /// Returns None if there is no active panel.
    pub fn active_panel(&self, cx: &App) -> Option<Arc<dyn PanelView>> {
        match &self.items {
            DockItem::Tabs { view, .. } => view.read(cx).active_panel(cx),
            DockItem::Split { items, .. } => {
                // For splits, return the active panel from the first tabs
                for item in items {
                    if let DockItem::Tabs { view, .. } = item {
                        return view.read(cx).active_panel(cx);
                    }
                }
                None
            }
            DockItem::Tiles { view, .. } => view.read(cx).active_panel(cx),
    ...
    /// Set the panel style of the dock area (mutating version).
    ///
    /// Use `PanelStyle::TabBar` to always show the tab bar, even for single tabs.
    pub fn set_panel_style(
        &mut self,
        style: PanelStyle,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ...
    /// Backward-compatible alias for `set_center`.
    ...
    /// Close the active tab in the center dock area.
    ///
    /// This closes whichever tab is currently active/visible in the center panel.
    /// Returns true if a tab was closed, false if no tab was active or closable.
    pub fn close_active_tab(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
        match &self.items {
            DockItem::Tabs { view, .. } => {
                view.update(cx, |tab_panel, cx| tab_panel.close_active_tab(window, cx))
            }
            DockItem::Split { items, .. } => {
                // For splits, try to close the active tab in the first tabs panel found
                for item in items {
                    if let DockItem::Tabs { view, .. } = item {
                        return view
                            .update(cx, |tab_panel, cx| tab_panel.close_active_tab(window, cx));
                    }
                }
                false
            }
    ...
    ///
    /// This will switch to the tab with the given panel name and ensure the dock is open.
    ...
    pub fn activate_panel(
        ...
        if let Some(dock) = dock {
            let result = dock.update(cx, |dock, cx| {
                // Get the DockItem from the dock
                if let DockItem::Tabs { items, view, .. } = &dock.panel {
                    // Find the panel index by name
                    let panel_ix = items.iter().position(|p| p.panel_name(cx) == panel_name);

                    if let Some(ix) = panel_ix {
                        // Update the active index in the TabPanel
                        view.update(cx, |tab_panel, cx| {
                            tab_panel.active_ix = ix;
                            cx.notify();
                        });
