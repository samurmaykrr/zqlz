...
/// Event emitted when a tab is right-clicked, allowing parent components
/// to show a context menu with appropriate actions.
#[derive(Clone, Debug)]
pub struct TabContextMenuEvent {
    /// The index of the tab that was right-clicked
    pub tab_index: usize,
    /// The position where the right-click occurred (for menu positioning)
...
impl TabPanel {
    ...
    /// Return the total number of panels in this TabPanel.
    ///
    /// This is useful for determining if tabs can be closed or for context menu logic.
    ...
    /// their state when a connection changes.
    ...
    ///
    /// Used by "Close Other Tabs" context menu option.
    ...
    ///
    /// Used by "Close Tabs to the Right" context menu option.
    ...
    fn render_toolbar(
        ...
        h_flex()
            ...
            .when(state.zoomable.is_some() || state.closable, |this| {
                this.child(
                    Button::new("menu")
    ...
    fn render_dock_toggle_button(
        ...
        let tooltip = {
            ...
            match (placement, is_open) {
                (DockPlacement::Left, true) => format!("{} ({}B)", t!("Dock.Collapse"), modifier),
                (DockPlacement::Left, false) => format!("{} ({}B)", t!("Dock.Expand"), modifier),
                (DockPlacement::Right, true) => format!("{} ({}⇧B)", t!("Dock.Collapse"), modifier),
                (DockPlacement::Right, false) => format!("{} ({}⇧B)", t!("Dock.Expand"), modifier),
                (DockPlacement::Bottom, true) => format!("{} ({}J)", t!("Dock.Collapse"), modifier),
                (DockPlacement::Bottom, false) => format!("{} ({}J)", t!("Dock.Expand"), modifier),
                _ => unreachable!(),
            }
        };
                    ...
                    Tab::new()
                        ...
                        .on_mouse_down(
                            ...
                            cx.listener(move |_view, event: &MouseDownEvent, _window, cx| {
                                // Emit event to let parent handle context menu
                                cx.emit(TabContextMenuEvent {
                                    tab_index: ix,
