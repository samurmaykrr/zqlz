    ...
    fn schedule_open(&mut self, cx: &mut Context<Self>) {
        ...
        self.open_task = Some(cx.spawn(async move |this, cx| {
            ...
            if let Err(error) = this.update(cx, |state, cx| {
                ...
                tracing::warn!("Failed to open hover card: {error}");
            }
        }));
    }
    ...
    fn schedule_close(&mut self, cx: &mut Context<Self>) {
        ...
        self.close_task = Some(cx.spawn(async move |this, cx| {
            ...
            if let Err(error) = this.update(cx, |state, cx| {
                ...
                tracing::warn!("Failed to close hover card: {error}");
            }
        }));
    }
...
impl RenderOnce for HoverCard {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let previous_open = state.read(cx).open;
        ...
        if let Some(content) = self.content {
            let content_element = state.update(cx, |state, cx| (content)(state, window, cx));
            popover_content = popover_content.child(content_element);
        }
