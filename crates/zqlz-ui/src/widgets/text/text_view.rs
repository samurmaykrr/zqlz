...
use crate::widgets::StyledExt;
use crate::widgets::scroll::ScrollableElement;
use crate::widgets::text::TextViewFormat;
use crate::widgets::text::node::CodeBlock;
use crate::widgets::text::state::TextViewState;
use crate::widgets::{global_state::GlobalState, text::TextViewStyle};
...
impl Element for TextView {
    ...
    fn request_layout(
        ...
        tracing::debug!(element_id = ?self.id, "TextView request_layout called");

        // Track whether we're using a pre-existing state (passed via TextView::new(&state))
        // vs creating/caching state internally (via TextView::markdown() or TextView::html())
        let using_external_state = self.state.is_some();

        let state = if let Some(state) = self.state.clone() {
            tracing::debug!("Using existing state");
            state
        } else {
            tracing::debug!("Creating new state");
            let default_format = self.format.unwrap_or(TextViewFormat::Markdown);
            let default_text = self.text.clone().unwrap_or_default();
            tracing::trace!(
                format = ?default_format,
                text_length = default_text.len(),
                "State initialization parameters"
            );

            tracing::trace!("Calling window.use_keyed_state");
            let state = window.use_keyed_state(
                SharedString::from(format!("{}/state", self.id)),
                cx,
                move |_, cx| {
                    tracing::trace!("Inside use_keyed_state initializer");
                    if default_format == TextViewFormat::Markdown {
                        tracing::debug!("Creating markdown state");
                        let result = TextViewState::markdown(default_text.as_str(), cx);
                        tracing::debug!("Markdown state created");
                        result
                    } else {
                        tracing::debug!("Creating html state");
                        let result = TextViewState::html(default_text.as_str(), cx);
                        tracing::debug!("HTML state created");
                        result
                    }
                },
            );
            tracing::debug!("State created via use_keyed_state");
            self.state = Some(state.clone());
            state
        };

        // Only update state properties when NOT using an external pre-configured state.
        // When using TextView::new(&state), the state is already configured by the caller,
        // and we should not modify it on every frame (which would cause render loops).
        if !using_external_state {
            tracing::debug!("Updating state");
            state.update(cx, |state, cx| {
                tracing::trace!("Inside state.update");
                ...
                if let Some(text) = self.text.clone() {
                    tracing::trace!(text_length = text.len(), "Calling state.set_text");
                    state.set_text(text.as_str(), cx);
                    tracing::trace!("state.set_text completed");
                }
                tracing::trace!("state.update complete");
            });
        }

        tracing::trace!("Reading focus_handle and list_state");
        let focus_handle = state.read(cx).focus_handle.clone();
        let list_state = state.read(cx).list_state.clone();
        tracing::trace!("Handles obtained");

        tracing::trace!("Creating div element");
        let mut el = div()
