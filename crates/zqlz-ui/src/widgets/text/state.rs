...
impl TextViewState {
    ...
    pub fn markdown(text: &str, cx: &mut Context<Self>) -> Self {
        tracing::debug!(text_len = text.len(), "Creating Markdown TextView");
        let result = Self::new(TextViewFormat::Markdown, text, cx);
        tracing::debug!("Markdown TextView created");
        result
    }

    /// Create a HTML TextViewState.
    pub fn html(text: &str, cx: &mut Context<Self>) -> Self {
        tracing::debug!(text_len = text.len(), "Creating HTML TextView");
        let result = Self::new(TextViewFormat::Html, text, cx);
        tracing::debug!("HTML TextView created");
        result
    }

    /// Create a new TextViewState.
    fn new(format: TextViewFormat, text: &str, cx: &mut Context<Self>) -> Self {
        tracing::debug!(format = ?format, text_len = text.len(), "Initializing TextViewState");
        let focus_handle = cx.focus_handle();

        let (tx, rx) = smol::channel::unbounded::<UpdateOptions>();
        let (tx_result, rx_result) = smol::channel::unbounded::<Result<(), SharedString>>();
        tracing::trace!("Spawning receive task");
        let _receive_task = cx.spawn({
            async move |weak_self, cx| {
                while let Ok(parsed_result) = rx_result.recv().await {
                    tracing::trace!(success = parsed_result.is_ok(), "Received parse result");
                    _ = weak_self.update(cx, |state, cx| {
                        if let Err(err) = &parsed_result {
                            tracing::debug!(error = %err, "Parse error occurred");
        ...
        tracing::trace!("Spawning parse task");
        let _parse_task = cx.background_spawn(UpdateFuture::new(format, rx, tx_result, cx));

        tracing::trace!("Creating TextViewState struct");
        let mut this = Self {
            focus_handle,
    ...
    fn increment_update(&mut self, text: &str, append: bool, cx: &mut Context<Self>) {
        ...
        tracing::trace!("Sending update options to parser");
        // Parse at first time by blocking.
        let send_result = self.tx.try_send(update_options);
        tracing::trace!(success = send_result.is_ok(), "Update options send result");
    }
...
fn parse_content(format: TextViewFormat, options: &UpdateOptions) -> Result<(), SharedString> {
    tracing::trace!(format = ?format, "Starting content parse");
    let mut node_cx = NodeContext {
        code_block_actions: options.code_block_actions.clone(),
        ..NodeContext::default()
    };

    tracing::trace!("Acquiring lock to extract source");
    let (source, is_append) = {
        let mut content = options.content.lock().unwrap();
        tracing::trace!("Lock acquired for source extraction");
        let mut source = String::new();
        let is_append = if options.append
        ...
        tracing::trace!(
            ...
            "Lock released after source extraction"
        );
        (source, is_append)
    };

    tracing::trace!("Parsing content");
    let new_content = match format {
        TextViewFormat::Markdown => {
            tracing::trace!("Calling markdown parser");
            let result = format::markdown::parse(&source, &mut node_cx, &options.highlight_theme);
            tracing::trace!(success = result.is_ok(), "Markdown parse result");
            result
        }
        TextViewFormat::Html => {
            tracing::trace!("Calling HTML parser");
            let result = format::html::parse(&source, &mut node_cx);
            tracing::trace!(success = result.is_ok(), "HTML parse result");
            result
        }
    }?;
    tracing::trace!("Content parsed successfully");

    tracing::trace!("Acquiring lock to update content");
    {
        let mut content = options.content.lock().unwrap();
        tracing::trace!("Lock acquired for content update");
        if is_append {
        ...
        tracing::trace!("Lock released after content update");
    }
    tracing::trace!("Content parse complete");
