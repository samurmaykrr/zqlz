...
impl InputState {
    ...
    pub(super) fn indent_inline(
        ...
        // If caller requested Tab to be emitted as an InputEvent (inline editors)
        // then send PressTab instead of performing indent behavior. This allows
        // hosts (like the table delegate) to intercept Tab and move selection.
        if self.emit_tab_event {
            cx.emit(crate::widgets::input::InputEvent::PressTab { shift: false });
            return;
        }
    ...
    pub(super) fn outdent_inline(
        ...
        if self.emit_tab_event {
            cx.emit(crate::widgets::input::InputEvent::PressTab { shift: true });
            return;
        }
