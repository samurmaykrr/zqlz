...
pub enum InputEvent {
    ...
    /// Tab or Shift+Tab pressed inside an Input
    PressTab {
        shift: bool,
    },
    /// Arrow key pressed - emitted when emit_arrow_event is true and cursor is at edge
    /// For Up/Down: always emitted in single-line mode
    /// For Left: emitted when cursor is at position 0
    /// For Right: emitted when cursor is at end of text
    PressArrow {
...
pub(crate) fn init(cx: &mut App) {
    cx.bind_keys([
        ...
        // Fold/Unfold bindings
        #[cfg(target_os = "macos")]
        KeyBinding::new("cmd-alt-[", FoldAtCursor, Some(CONTEXT)),
        #[cfg(not(target_os = "macos"))]
        KeyBinding::new("ctrl-shift-[", FoldAtCursor, Some(CONTEXT)),
        #[cfg(target_os = "macos")]
        KeyBinding::new("cmd-alt-]", UnfoldAtCursor, Some(CONTEXT)),
        #[cfg(not(target_os = "macos"))]
        KeyBinding::new("ctrl-shift-]", UnfoldAtCursor, Some(CONTEXT)),
        #[cfg(target_os = "macos")]
...
pub struct InputState {
    ...
    /// Diagnostic popover data and cached text view state
    diagnostic_popover_data: Option<DiagnosticPopoverData>,
    diagnostic_text_view_state: Option<Entity<TextViewState>>,
    ...
    /// When true the Input will emit InputEvent::PressTab instead of performing
    /// the normal indent/outdent behavior. Used by callers embedding Input as
    /// an inline cell editor (e.g. table delegate) to intercept Tab/Shift+Tab.
    pub(super) emit_tab_event: bool,

    /// When true the Input will emit InputEvent::PressArrow when:
...
impl InputState {
    ...
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        tracing::debug!(input_state_id = id, "Creating InputState");
        ...
        Self {
            ...
            lsp: Lsp::default(),
            diagnostic_popover_data: None,
            diagnostic_text_view_state: None,
            context_menu: None,
            mouse_context_menu,
            ...
            emit_tab_event: false,
            emit_arrow_event: false,
    ...
    pub fn code_editor(mut self, language: impl Into<SharedString>) -> Self {
        ...
        tracing::debug!(language = %language, "Setting code editor mode");
        self.mode = InputMode::code_editor(language.clone());
    ...
    /// When true causes Tab/Shift+Tab to emit InputEvent::PressTab instead of
    /// performing indent/outdent. This is useful when the input is used as an
    /// inline cell editor (table) so the table can commit and move selection.
    pub fn emit_tab_event(mut self, emit: bool) -> Self {
        self.emit_tab_event = emit;
        self
    }

    /// When true causes arrow keys to emit InputEvent::PressArrow at boundaries:
    ...
    pub fn set_inline_diagnostics(
        ...
        self.mode
            .set_inline_diagnostics(show_inline_diagnostics, _window, cx);
    ...
    fn handle_diagnostic_popover(&mut self, offset: usize, cx: &mut Context<Self>) {
        ...
        let Some(entry) = diagnostics.for_offset(offset) else {
            if self.diagnostic_popover_data.is_some() {
                self.diagnostic_popover_data = None;
                self.diagnostic_text_view_state = None;
                cx.notify();
        ...
        if self
            .diagnostic_popover_data
            .as_ref()
            .map(|data| data.matches(entry))
            .unwrap_or(false)
        {
            return;
        }

        let data = DiagnosticPopoverData::from_entry(entry);
        let text_view_state = cx.new(|cx| TextViewState::markdown(&data.message, cx));
        self.diagnostic_popover_data = Some(data);
        self.diagnostic_text_view_state = Some(text_view_state);
        cx.notify();
    }
    ...
    /// Return the selection range (start, end) in byte offsets.
    ...
    pub(super) fn enter(&mut self, action: &Enter, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // When secondary modifier is held (cmd on macOS, ctrl on other platforms),
        // don't insert a newline - just emit the event for the parent to handle
        // (e.g., execute query on cmd+enter)
        if action.secondary {
    ...
    pub(super) fn fold_at_cursor(
        &mut self,
        _action: &FoldAtCursor,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if !self.mode.is_code_editor() {
        ...
        let cursor_line = self.text.byte_to_line_idx(self.cursor(), LineType::LF);
        self.fold_state.toggle(cursor_line);
        cx.notify();
    }

    pub(super) fn unfold_at_cursor(
        &mut self,
        _action: &UnfoldAtCursor,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        if !self.mode.is_code_editor() {
        ...
        let cursor_line = self.text.byte_to_line_idx(self.cursor(), LineType::LF);
        self.fold_state.unfold(cursor_line);
        cx.notify();
    }
    ...
    pub(super) fn on_mouse_move(
        ...
        self.handle_diagnostic_popover(offset, cx);
    }
    ...
    pub(super) fn on_scroll_wheel(
        ...
        // Clear inline popovers on scroll
        self.clear_inline_popovers();
    }
    ...
    /// Render the diagnostic popover if there's data for it
    fn render_diagnostic_popover(&self, _cx: &App) -> Option<impl IntoElement> {
        if !self.mode.show_inline_diagnostics() {
            return None;
        }
        ...
        let (_, _, start_pos) = self.line_and_position_for_offset(data.range.start);
        let (_, _, end_pos) = self.line_and_position_for_offset(data.range.end);

        let start_pos = start_pos?;
        let end_pos = end_pos?;

        let origin = diagnostic_popover_origin(
            data.range.start,
            data.range.end,
...
impl super::popovers::CompletionMenuEditor for InputState {
    ...
    fn completion_cursor_offset(&self, _cx: &App) -> usize {
        self.cursor()
    }

    fn completion_text_string(&self, _cx: &App) -> String {
        self.text.to_string()
    }
    ...
    fn completion_origin(&self, _cx: &App) -> Option<Point<Pixels>> {
        ...
        Some(
            scroll_origin + cursor_origin - self.input_bounds.origin
...
impl EntityInputHandler for InputState {
    ...
    fn replace_text_in_range(
        ...
        // Update fold state for code editor mode
        if self.mode.is_code_editor() {
            self.fold_state = FoldState::detect_sql_regions(&self.text);
        }
...
impl Render for InputState {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        tracing::trace!(
            ...
            "Rendering InputState"
        );
        if self._pending_update {
            tracing::debug!("Performing pending highlighter update in render");
            self.mode
        ...
        div()
            ...
            .flex_grow()
            .overflow_x_hidden()
            .on_mouse_move(window.listener_for(&cx.entity(), InputState::on_mouse_move))
