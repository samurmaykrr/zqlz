...
impl InputState {
    pub(crate) fn handle_completion_trigger(
        ...
        // If start is 2+ positions ahead, we've moved to a new word (with space in between)
        let start_offset = if let Some(existing) = menu_trigger {
            if start > existing && (start - existing) > 1 {
                // Jumped ahead (space was typed) - reset to new word
                tracing::warn!(
                    "🔄 New word detected! Resetting trigger: old={}, new={} (gap={})",
                    existing,
                    start,
                    start - existing
                );
                ...
                // Continuing same word - keep existing trigger
                existing
            }
        } else {
            // First time - use start
            start
        };
        ...
        tracing::warn!(
            "COMPLETION TRIGGER: start={}, start_offset={}, new_offset={}, cursor={}",
            start,
            start_offset,
            new_offset,
            self.cursor()
