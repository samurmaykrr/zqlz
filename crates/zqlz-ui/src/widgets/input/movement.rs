...
use crate::widgets::input::{
    ArrowDirection, InputEvent, InputState, MoveDown, MoveEnd, MoveHome, MoveLeft, MovePageDown,
    MovePageUp, MoveRight, MoveToEnd, MoveToNextWord, MoveToPreviousWord, MoveToStart, MoveUp,
    RopeExt as _,
};
...
impl InputState {
    ...
    pub(super) fn left(&mut self, _: &MoveLeft, _: &mut Window, cx: &mut Context<Self>) {
        // Emit arrow event if cursor is at the beginning of text
        if self.emit_arrow_event && self.cursor() == 0 && self.selected_range.is_empty() {
            cx.emit(InputEvent::PressArrow {
    ...
    pub(super) fn right(&mut self, _: &MoveRight, _: &mut Window, cx: &mut Context<Self>) {
        // Emit arrow event if cursor is at the end of text
        let text_len = self.text.len();
        if self.emit_arrow_event && self.cursor() == text_len && self.selected_range.is_empty() {
            cx.emit(InputEvent::PressArrow {
    ...
    pub(super) fn up(&mut self, action: &MoveUp, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.mode.is_single_line() {
            // In single-line mode, emit arrow event for cell navigation if enabled
            if self.emit_arrow_event {
                cx.emit(InputEvent::PressArrow {
