...
impl TextElement {
    ...
    fn layout_cursor(
            ...
            (cursor_pos, cursor_start, cursor_end)
        {
            ...
            let cursor_height = match state.size {
                crate::widgets::Size::Large => 1.,
                crate::widgets::Size::Small => 0.75,
                _ => 0.85,
            } * line_height;
    ...
    fn highlight_lines(
        ...
        let (highlighter, diagnostics, show_inline_diagnostics) = match &state.mode {
            InputMode::CodeEditor {
                highlighter,
                diagnostics,
                show_inline_diagnostics,
                ..
            } => (highlighter.borrow(), diagnostics, *show_inline_diagnostics),
            _ => return None,
        };
            ...
            .take(visible_range.len())
        {
            ...
            tracing::trace!(
                ...
                range_end = range.end,
                theme = %highlight_theme.name,
                "Requesting syntax highlight styles"
            );
            let line_styles = highlighter.styles(&range, highlight_theme);
            tracing::trace!(
                styles_count = line_styles.len(),
                "Received highlight styles"
            );
        ...
        // Only apply diagnostic squiggles if show_inline_diagnostics is enabled
        let diagnostic_styles = if show_inline_diagnostics {
            diagnostics.styles_for_range(&visible_byte_range, cx)
        } else {
            vec![]
        };
...
pub(super) struct PrepaintState {
    ...
    /// Diagnostic severity for each visible line (indexed by visible line index, not absolute line)
    /// Used to paint error/warning indicators in the gutter.
    line_diagnostics: Vec<Option<DiagnosticSeverity>>,
    ...
    /// Fold regions (map from start line to region info)
    fold_regions: std::collections::HashMap<usize, bool>,
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        // Build line diagnostics for gutter indicators
        let line_diagnostics: Vec<Option<DiagnosticSeverity>> =
            if state.mode.show_inline_diagnostics() && line_numbers.is_some() {
                ...
                last_layout
                    ...
                    .enumerate()
                    .map(|(ix, _)| {
                        let absolute_line = last_layout.visible_range.start + ix;
                        diagnostics.and_then(|d| d.severity_for_line(absolute_line))
                    })
                    .collect()
            } else {
                Vec::new()
        ...
        // Build fold indicators for code editor mode
        let mut fold_regions = std::collections::HashMap::new();
        let mut folded_lines = std::collections::HashSet::new();
        if state.mode.is_code_editor() {
            for region in state.fold_state.regions() {
                if last_layout.visible_range.contains(&region.start_line) {
                    let is_folded = state.fold_state.is_folded(region.start_line);
                    fold_regions.insert(region.start_line, is_folded);
                    if is_folded {
                        folded_lines.insert(region.start_line);
                    }
                }
            }
        }
    ...
    fn paint(
        ...
        if let Some(line_numbers) = prepaint.line_numbers.as_ref() {
            ...
            for (ix, lines) in line_numbers.iter().enumerate() {
                ...
                // Paint diagnostic indicator (colored dot) in the gutter
                if let Some(Some(severity)) = prepaint.line_diagnostics.get(ix) {
                    let indicator_size = px(6.0);
                    let indicator_margin = px(4.0);
                    let indicator_color = match severity {
                        DiagnosticSeverity::Error => cx.theme().danger,
                        DiagnosticSeverity::Warning => cx.theme().warning,
                        DiagnosticSeverity::Info => cx.theme().info,
                        DiagnosticSeverity::Hint => cx.theme().muted_foreground,
                    };
                    let indicator_bounds = Bounds::new(
                        point(
                            input_bounds.origin.x + indicator_margin,
                            origin.y + offset_y + (line_height - indicator_size) / 2.0,
                        ),
                        size(indicator_size, indicator_size),
                    );
                    window.paint_quad(
                        fill(indicator_bounds, indicator_color).corner_radii(indicator_size / 2.0),
                    );
                }

                // Paint fold indicator (chevron) in the gutter for code editor mode
                if let Some(&is_folded) = prepaint.fold_regions.get(&row) {
                    let indicator_size = px(8.0);
                    let indicator_margin = prepaint.last_layout.line_number_width
                        - LINE_NUMBER_RIGHT_MARGIN
                        - px(12.0);
                    let indicator_center = point(
                        input_bounds.origin.x + indicator_margin + indicator_size / 2.0,
                        origin.y + offset_y + line_height / 2.0,
                    );

                    // Draw triangle (right-pointing if folded, down-pointing if unfolded)
                    let mut path = Path::new(indicator_center);
                    let half_size = indicator_size / 2.0;

                    if is_folded {
                        // Right-pointing triangle: ▶
                        path.line_to(point(indicator_center.x + half_size, indicator_center.y));
                        path.line_to(point(indicator_center.x, indicator_center.y - half_size));
                        path.line_to(point(indicator_center.x, indicator_center.y + half_size));
                    } else {
                        // Down-pointing triangle: ▼
                        path.line_to(point(
                            indicator_center.x - half_size,
                            indicator_center.y - half_size / 2.0,
                        ));
                        path.line_to(point(
                            indicator_center.x + half_size,
                            indicator_center.y - half_size / 2.0,
            ...
            && let Some(cursor_bounds) = prepaint.cursor_bounds_with_scroll()
        {
            ...
            let _ = first_line.paint(p, line_height, TextAlign::Left, None, window, cx);
        }
