...
use crate::widgets::highlighter::DiagnosticSet;
use crate::widgets::highlighter::SyntaxHighlighter;
use crate::widgets::input::{InputState, RopeExt as _, TabSize};
...
pub(crate) enum InputMode {
    ...
    CodeEditor {
        ...
        /// Show line number
        line_number: bool,
        /// Show inline diagnostics and hover popovers.
...
impl InputMode {
    ...
    pub(super) fn set_inline_diagnostics(
        &mut self,
        show_inline_diagnostics: bool,
        _window: &mut Window,
        cx: &mut Context<InputState>,
    ) {
        debug_assert!(self.is_code_editor());
        if let InputMode::CodeEditor {
            show_inline_diagnostics: current,
            ..
        } = self
        {
            if *current == show_inline_diagnostics {
                return;
            }
            *current = show_inline_diagnostics;
        }
        cx.notify();
    }

    #[inline]
    pub(super) fn show_inline_diagnostics(&self) -> bool {
        match self {
            InputMode::CodeEditor {
                show_inline_diagnostics,
                multi_line,
                ..
            } => *show_inline_diagnostics && *multi_line,
            _ => false,
        }
    }
        ...
        match &self {
            InputMode::CodeEditor {
                ...
                if highlighter.is_none() {
                    tracing::debug!(language = %language, "Initializing SyntaxHighlighter");
