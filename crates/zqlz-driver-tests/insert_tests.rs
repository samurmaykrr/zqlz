...
async fn test_insert_actor_basic(#[case] driver: TestDriver) -> Result<()> {
    ...
    // Clean up
    conn.execute(
        "DELETE FROM actor WHERE first_name = 'Test' AND last_name = 'Actor'",
        &[],
    )
    .await?;
...
async fn test_insert_actor_returning_if_supported(#[case] driver: TestDriver) -> Result<()> {
    ...
    let sql =
        "INSERT INTO actor (first_name, last_name) VALUES ('Return', 'Test') RETURNING actor_id";
...
async fn test_insert_actor_batch(#[case] driver: TestDriver) -> Result<()> {
    ...
    let count = verify_result.rows[0]
        .get_by_name("count")
        .and_then(|v| v.as_i64())
    ...
    // Clean up
    conn.execute("DELETE FROM actor WHERE first_name = 'Batch'", &[])
        .await?;
...
async fn test_insert_actor_not_null_violation(#[case] driver: TestDriver) -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Should fail due to NOT NULL constraint on last_name"
    );
...
async fn test_insert_actor_special_characters(#[case] driver: TestDriver) -> Result<()> {
    ...
    assert_eq!(
        verify_result.rows.len(),
        1,
        "Should find the inserted actor"
    );
    ...
    assert_eq!(
        first_name, "John's",
        "First name should preserve apostrophe"
    );
    assert_eq!(last_name, "O'Brien", "Last name should preserve apostrophe");

    // Clean up
    conn.execute("DELETE FROM actor WHERE first_name = 'John''s'", &[])
        .await?;
...
async fn test_insert_actor_unicode(#[case] driver: TestDriver) -> Result<()> {
    ...
    // Clean up
    conn.execute("DELETE FROM actor WHERE first_name = '山田'", &[])
        .await?;
...
async fn test_insert_film_minimal_required_fields(#[case] driver: TestDriver) -> Result<()> {
    ...
    let language_id = lang_result.rows[0]
        .get_by_name("language_id")
        .and_then(|v| v.as_i64())
    ...
    let sql = match driver {
        TestDriver::Postgres => "INSERT INTO film (title, language_id) VALUES ('Test Film', $1)",
    ...
    // Clean up
    conn.execute("DELETE FROM film WHERE title = 'Test Film'", &[])
        .await?;
...
#[case::sqlite(TestDriver::Sqlite)]
#[tokio::test]
async fn test_insert_customer_with_valid_store_and_address(
    ...
    let store_id = store_result.rows[0]
    ...
    let address_id = address_result.rows[0]
        .get_by_name("address_id")
        .and_then(|v| v.as_i64())
    ...
    let result = conn
        .execute(sql, &[Value::Int64(store_id), Value::Int64(address_id)])
        .await?;

    assert_eq!(result.affected_rows, 1, "Should insert one row");

    // Clean up
    conn.execute("DELETE FROM customer WHERE email = 'test@example.com'", &[])
        .await?;
...
async fn test_insert_inventory_valid_film(#[case] driver: TestDriver) -> Result<()> {
    ...
    let film_id = film_result.rows[0]
        .get_by_name("film_id")
        .and_then(|v| v.as_i64())
    ...
    let sql = match driver {
        TestDriver::Postgres => "INSERT INTO inventory (film_id, store_id) VALUES ($1, $2)",
        TestDriver::Mysql | TestDriver::Sqlite => {
    ...
    let result = conn
        .execute(sql, &[Value::Int64(film_id), Value::Int64(store_id)])
        .await?;
    ...
    conn.execute(cleanup_sql, &[Value::Int64(film_id), Value::Int64(film_id)])
        .await?;
...
async fn integration_test_insert_works() -> Result<()> {
    ...
    conn.execute(
        "CREATE TEMP TABLE test_insert (id INTEGER PRIMARY KEY, name TEXT)",
        &[],
    )
    .await?;

    let result = conn
        .execute("INSERT INTO test_insert (name) VALUES ('test')", &[])
        .await?;
    ...
    let verify = conn
        .query("SELECT name FROM test_insert WHERE name = 'test'", &[])
        .await?;
